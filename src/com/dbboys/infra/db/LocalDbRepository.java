...
public  class LocalDbRepository {
    ...
    public static void migrateTConnectTable() {
        ...
        execIgnore("create table if not exists t_migration_task_run_item(c_id INTEGER PRIMARY KEY AUTOINCREMENT,c_task_id INTEGER,c_kind varchar(20),c_name varchar(500),c_status varchar(20),c_start_time varchar(20),c_end_time varchar(20),c_source_rows INTEGER,c_target_rows INTEGER,c_speed INTEGER,c_elapsed INTEGER,c_progress REAL,c_error varchar(3200),c_error_sql varchar(3200),c_checksum varchar(100),c_info varchar(3200))");
   ...
   */
    ...
    public static List<MigrationTask> getAllMigrationTasks() {
        ...
        try (Statement stmt = conn.createStatement();
            ...
            while (rs.next()) {
                ...
                task.setLastStartTime(rs.getString("c_last_start_time") != null ? rs.getString("c_last_start_time") : "");
                task.setLastEndTime(rs.getString("c_last_end_time") != null ? rs.getString("c_last_end_time") : "");
    ...
    public static boolean deleteMigrationTask(MigrationTask task) {
        try (PreparedStatement ps = conn.prepareStatement("DELETE FROM t_migration_task WHERE c_id=?")) {
            deleteMigrationTaskRunItemsByTask(task.getId());
    ...
    /** 更新任务最近一次运行结果（起止时间+按类型成功失败计数 JSON），不影响任务配置列。 */
    public static boolean updateMigrationTaskRunInfo(int taskId, String startTime, String endTime, String runCountsJson) {
        try (PreparedStatement ps = conn.prepareStatement(
                "UPDATE t_migration_task SET c_last_start_time=?, c_last_end_time=?, c_run_counts=? WHERE c_id=?")) {
            ps.setString(1, startTime);
            ps.setString(2, endTime);
            ps.setString(3, runCountsJson);
            ps.setInt(4, taskId);
            ps.executeUpdate();
            return true;
        } catch (Exception e) {
            log.error("Failed to update migration task run info", e);
            return false;
        }
    }

    public static boolean deleteMigrationTaskRunItemsByTask(int taskId) {
        try (PreparedStatement ps = conn.prepareStatement(
    ...
    public static boolean deleteMigrationTaskRunItemsByParent(int folderId) {
        try (PreparedStatement ps = conn.prepareStatement(
    ...
    // ---- MIGRATION TASK RUN ITEM（逐对象明细，每次运行整批替换） ----
    public static boolean createMigrationTaskRunItems(List<MigrationTaskRunItem> items) {
        ...
        String sql = "INSERT INTO t_migration_task_run_item (c_task_id, c_kind, c_name, c_status, "
                + "c_start_time, c_end_time, c_source_rows, c_target_rows, c_speed, c_elapsed, c_progress, "
                + "c_error, c_error_sql, c_checksum, c_info) "
                + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            for (MigrationTaskRunItem item : items) {
                ...
                ps.setLong(10, item.getElapsed());
                ps.setDouble(11, item.getProgress());
                ps.setString(12, item.getError());
                ps.setString(13, item.getErrorSql());
                ps.setString(14, item.getChecksum());
                ps.setString(15, item.getInfo());
                ps.addBatch();
            }
            ps.executeBatch();
            return true;
    ...
    public static List<MigrationTaskRunItem> getMigrationTaskRunItems(int taskId) {
        ...
        String sql = "SELECT c_id, c_task_id, c_kind, c_name, c_status, c_start_time, c_end_time, "
                + "c_source_rows, c_target_rows, c_speed, c_elapsed, c_progress, c_error, c_error_sql, c_checksum, c_info "
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    long elapsed = rs.getLong("c_elapsed");
                    item.setElapsed(rs.wasNull() ? -1 : elapsed);
                    double progress = rs.getDouble("c_progress");
                    item.setProgress(rs.wasNull() ? -1 : progress);
                    item.setError(rs.getString("c_error") != null ? rs.getString("c_error") : "");
                    item.setErrorSql(rs.getString("c_error_sql") != null ? rs.getString("c_error_sql") : "");
            ...
            log.error("Failed to load migration task run items", e);
        }
        return list;
    }
