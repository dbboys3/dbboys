...
public final class TypeMapper {
    ...
    public static String buildCreateTableScript(String sourceDbType, String targetDbType,
        ...
        for (ColumnsInfo column : columns) {
            ...
            // 自增子句（类型替换已处理的除外）。Oracle 系的 IDENTITY 是列定义子句，
            // 语法上必须位于 NOT NULL 约束之前（NOT NULL 在前会报 ORA-00922），故提前拼接
            boolean oracleIdentity = auto && !autoHandledByType && oracleFamily;
            if (oracleIdentity) {
            ...
            if (auto && !autoHandledByType && !oracleIdentity) {
                if (mysql) {
