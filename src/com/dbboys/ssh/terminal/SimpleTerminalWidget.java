...
public class SimpleTerminalWidget extends Canvas {
    ...
    private byte[] key(KeyEvent e) {
        ...
        if (ct) {
            if (k == KeyCode.C) { String s = selectedText(); if (!s.isEmpty()) Clipboard.getSystemClipboard().setContent(java.util.Collections.singletonMap(DataFormat.PLAIN_TEXT, s)); return new byte[]{0x03}; }
