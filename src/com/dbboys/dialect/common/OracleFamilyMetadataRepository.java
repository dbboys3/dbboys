...
import com.dbboys.model.Synonym;
...
/**
 * Shared metadata implementation for the Oracle family of dialects (Oracle, Dameng).
 * Dialect differences are isolated behind the hook methods below; everything else is verbatim shared.
 */
public abstract class OracleFamilyMetadataRepository implements MetadataRepository {
    private static final int DEFAULT_QUERY_TIMEOUT_SECONDS = 30;
    private static final String ZERO_SIZE = "0B";
    private static final int FLAGS = 16384;
    private static final int PROC_FLAGS = 4;

    // ------------------------------------------------------------------
    // Dialect hooks
    // ------------------------------------------------------------------

    /** Product name used as schema/database fallback ({@code "ORACLE"} / {@code "DAMENG"}). */
    protected abstract String dialectName();

    /** Resolves the current schema; dialects differ in JDBC-first vs query-first order. */
    protected abstract String currentSchema(Connection conn) throws SQLException;

    /** True when the error means a probed dictionary view/object is missing (ORA-00942 and dialect equivalents). */
    protected abstract boolean isOra942ObjectNotExists(Throwable t);

    /** Switches the session current schema (behind {@link #changeDatabase}/{@link #setDatabase}). */
    protected abstract void applyCurrentSchema(Connection conn, String databaseName) throws SQLException;

    /** Instance info query behind {@code loadCurrentDatabase}. */
    protected abstract String sqlCurrentDatabase();

    /** Non-system schema names. */
    protected abstract String sqlUsers();

    /** Session service-name / locale probe for the schema list. */
    protected abstract String sqlSessionSchemaListContext();

    /** Schemas with total segment size. */
    protected abstract String sqlUsersWithSize();

    /** Schemas with creation date. */
    protected abstract String sqlUsersWithCreated();

    /** Single-schema info row behind {@code loadSchemaInfo}. */
    protected abstract String sqlSchemaInfo();

    /** User tables of a schema. */
    protected abstract String sqlUserTables();

    /** Sequences of a schema. */
    protected abstract String sqlSequences();

    /** Synonyms of a schema. */
    protected abstract String sqlSynonyms();

    /** Triggers of a schema. */
    protected abstract String sqlTriggers();

    /** Single trigger detail. */
    protected abstract String sqlTriggerDetail();

    /** Data-dictionary view count (system objects). */
    protected abstract String sqlDictViewCount();

    /** Views of a schema. */
    protected abstract String sqlViews();

    /** Object-type count of a schema. */
    protected abstract String sqlTypeCount();

    /** Object types of a schema. */
    protected abstract String sqlTypes();

    protected static final String SQL_CURRENT_SCHEMA = """
    ...
    public List<User> getUsers(Connection conn) throws SQLException {
        ...
        return runner.query(sqlUsers(), null, rs -> new User(rs.getString(1)));
    }
    ...
    private SessionSchemaListContext loadSessionSchemaListContext(SqlRunner runner) throws SQLException {
        try {
            SessionSchemaListContext ctx = runner.queryOne(sqlSessionSchemaListContext(), null, rs ->
    ...
    public int getSystemTablesCount(Connection conn) throws SQLException {
        SqlRunner runner = runner(conn);
        Integer value = runner.queryOne(sqlDictViewCount(), null, rs -> rs.getInt(1));
        return value == null ? 0 : value;
    }
    ...
    public List<Table> getUserTables(Connection conn, String databaseName) throws SQLException {
        ...
        try {
            return runner.query(sqlUserTables(), List.of(owner), rs -> mapTable(rs, owner, includeSize));
        } catch (SQLException e) {
    ...
    public ArrayList<ColumnsInfo> getColumns(Connection conn, String tableName) throws SQLException {
        ...
        List<ColumnsInfo> rows = runner.query(SQL_COLUMNS, List.of(owner, objectName.objectName()), rs -> {
            ...
            String defNorm = normalizeColumnDefault(rs.getString("default_value"));
    ...
    public List<Sequence> getSequences(Connection conn, String databaseName) throws SQLException {
        SqlRunner runner = runner(conn);
        String owner = currentSchema(conn);
        return runner.query(sqlSequences(), List.of(owner), rs -> mapSequence(rs, owner));
    }
    ...
    public List<Synonym> getSynonyms(Connection conn, String databaseName) throws SQLException {
        SqlRunner runner = runner(conn);
        String owner = currentSchema(conn);
        return runner.query(sqlSynonyms(), List.of(owner), rs -> mapSynonym(rs, owner));
    }
    ...
    public List<Trigger> getTriggers(Connection conn, String databaseName) throws SQLException {
        SqlRunner runner = runner(conn);
        String owner = currentSchema(conn);
        return runner.query(sqlTriggers(), List.of(owner), rs -> mapTrigger(rs, owner));
    }
    ...
    public Trigger getTrigger(Connection conn, String databaseName, String triggerName) throws SQLException {
        ...
        SqlRunner runner = runner(conn);
        String owner = resolveOwner(conn, objectName.owner());
        return runner.queryOne(sqlTriggerDetail(), List.of(owner, objectName.objectName()), rs -> mapTrigger(rs, owner));
    }
    ...
    public List<View> getViews(Connection conn, String databaseName) throws SQLException {
        SqlRunner runner = runner(conn);
        String owner = currentSchema(conn);
        return runner.query(sqlViews(), List.of(owner), rs -> {
    ...
    public List<Procedure> getProcedures(Connection conn, String databaseName, boolean filterType) throws SQLException {
        ...
        return runner.query(SQL_PROCEDURES, List.of(owner), rs -> {
            ...
            procedure.setRows(0);
            procedure.setProcFlags(PROC_FLAGS);
            return procedure;
        });
    }
    ...
    public int getObjectTypeCount(Connection conn, String databaseName) throws SQLException {
        ...
        String owner = (databaseName == null || databaseName.isBlank()) ? currentSchema(conn) : databaseName;
        try {
            Integer value = runner.queryOne(sqlTypeCount(), List.of(owner), rs -> rs.getInt(1));
            return value == null ? 0 : value;
        } catch (SQLException e) {
            if (isOra942ObjectNotExists(e)) {
                return 0;
    ...
    public List<Type> getObjectTypes(Connection conn, String databaseName) throws SQLException {
        ...
        String owner = (databaseName == null || databaseName.isBlank()) ? currentSchema(conn) : databaseName;
        try {
            return runner.query(sqlTypes(), List.of(owner), rs -> {
    ...
    protected SqlRunner runner(Connection conn) {
    ...
    private Catalog loadCurrentDatabase(Connection conn) throws SQLException {
        ...
        SqlRunner runner = runner(conn);
        Catalog database = runner.queryOne(sqlCurrentDatabase(), null, rs -> {
    ...
    private String sessionUser(Connection conn) throws SQLException {
        ...
        return blankToFallback(sessionUser, dialectName());
    }
    ...
    private Catalog mapSchemaDatabase(String schemaName) {
        Catalog database = new Catalog(blankToFallback(schemaName, dialectName()));
    ...
    private Table mapTable(java.sql.ResultSet rs, String databaseName, boolean includeSize) throws SQLException {
        ...
        applyTableLoggingType(table, rs);
        table.setLockType("");
    ...
    private static String normalizeColumnDefault(String raw) {
        if (raw == null) {
            return "";
        }
    ...
    protected String blankToFallback(String value, String fallback) {
