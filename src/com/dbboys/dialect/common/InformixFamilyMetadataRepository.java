 ...
 */
public abstract class InformixFamilyMetadataRepository implements com.dbboys.core.MetadataRepository {
    ...
    /** Built-in system owner name ({@code "gbasedbt"} / {@code "informix"}); used in system-catalog SQL and user checks. */
    protected abstract String systemOwnerName();

    /** First-character expression for an index-name column: {@code LEFT(col,1)} (GBase) or {@code col[1,1]} (Informix). */
    protected abstract String idxFirstChar(String column);

    /** Table-comment query; empty string means table comments are unsupported (always returns {@code ""}). */
    protected abstract String sqlTableComment();

    /** Extra statement run after {@code database ?} in {@link #setDatabase} (e.g. GBase sqlmode); {@code null} = none. */
    ...
    private static final String SQL_DATABASES_DEFAULT = """
            ...
            order by
            case when name in ('sysmaster','sysuser','sysadmin','sysutils','sysha','syscdr','syscdcv1','%s','sys')
    ...
    private static final String SQL_INDEX_COUNT = """
            select count(*) from sysindexes i,systables t where i.tabid=t.tabid and t.tabid>(SELECT tabid FROM systables WHERE tabname = ' VERSION') and %s != ' '
            """;
    ...
    private static final String SQL_INDEX = """
            ...
            from
            systables t join sysindexes i
            on t.tabid = i.tabid and i.idxname==? and %2$s != ' '
            join sysobjstate o on  o.tabid=t.tabid and o.name=i.idxname
            left join sysmaster:systabnames st
    ...
    public List<Database> getDatabases(Connection conn) throws SQLException {
        try {
            ...
            if (e != null && e.getErrorCode() == -201) {
                return queryDatabases(conn, SQL_DATABASES_FALLBACK.formatted(systemOwnerName()));
            }
            throw e;
        }
    }
    ...
    public String getTableComment(Connection conn, String tableName) throws SQLException {
        ...
        SqlRunner runner = new SqlRunner(conn, DEFAULT_QUERY_TIMEOUT_SECONDS);
        String comment = runner.queryOne(sql, List.of(tableName), rs -> rs.getString(1));
        return comment == null ? "" : comment;
    }

    public List<Index> getIndexes(Connection conn, String databaseName) throws SQLException {
        SqlRunner runner = new SqlRunner(conn, DEFAULT_QUERY_TIMEOUT_SECONDS);
        String sql = SQL_INDEXES.formatted(toSqlStringLiteral(databaseName), idxFirstChar("i.idxname"));
        return runner.query(sql, List.of(databaseName), rs -> {
    ...
    public int getIndexCount(Connection conn) throws SQLException {
        SqlRunner runner = new SqlRunner(conn, DEFAULT_QUERY_TIMEOUT_SECONDS);
        Integer value = runner.queryOne(SQL_INDEX_COUNT.formatted(idxFirstChar("i.idxname")), null, rs -> rs.getInt(1));
        return value == null ? 0 : value;
    }

    public String getIndexSize(Connection conn) throws SQLException {
        SqlRunner runner = new SqlRunner(conn, DEFAULT_QUERY_TIMEOUT_SECONDS);
        return runner.queryOne(SQL_INDEX_SIZE.formatted(idxFirstChar("s.idxname")), null, rs -> rs.getString(1));
    }

    public Index getIndex(Connection conn, String databaseName, String indexName) throws SQLException {
        SqlRunner runner = new SqlRunner(conn, DEFAULT_QUERY_TIMEOUT_SECONDS);
        String sql = SQL_INDEX.formatted(toSqlStringLiteral(databaseName), idxFirstChar("i.idxname"));
    ...
    public void setDatabase(Connection conn, String databaseName) throws SQLException {
        ...
        String postStatement = postSetDatabaseStatement();
        if (postStatement != null) {
            try (java.sql.PreparedStatement pstmt = conn.prepareStatement(postStatement)) {
                pstmt.executeUpdate();
            }
            catch (SQLException e) {
                // ignore
    ...
    protected static String toSqlStringLiteral(String value) {
        if (value == null) {
            return "''";
        }
